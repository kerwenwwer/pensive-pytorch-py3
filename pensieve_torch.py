...
def central_agent(net_params_queues, exp_queues, model_type,  barrier):
    ...
    for epoch in range(TOTALEPOCH):
        ...
        # Barrier thead after all thread finish 
...
if __name__ == '__main__':
    ...
    barrier = mp.Barrier(NUM_AGENTS+1)
    # create a coordinator and multiple agent processes
    # (note: threading is not desirable due to python GIL)

    coordinator = mp.Process(target=central_agent,
                             args=(net_params_queues, exp_queues,arglist.model_type, barrier))
    coordinator.start()
    ...
    for i in range(NUM_AGENTS):
        agents.append(mp.Process(target=agent,
                                 args=(i, all_cooked_time, all_cooked_bw,
                                       net_params_queues[i],
                                       exp_queues[i],arglist.model_type, barrier)))
        agents[i].start()
