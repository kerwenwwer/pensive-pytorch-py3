...
class A3C(object):
    def __init__(self,is_central,model_type,s_dim,action_dim,actor_lr=1e-4,critic_lr=1e-3):
        ...
        #self.actorNetwork=ActorNetwork(self.s_dim,self.a_dim).cuda()
        ...
        if self.is_central:
            ...
            if model_type<2:
                ...
                #self.criticNetwork=CriticNetwork(self.s_dim,self.a_dim).cuda()
    ...
    def getNetworkGradient(self,s_batch,a_batch,r_batch,terminal):
        ...
        # s_batch=torch.cat(s_batch).cuda()
        # a_batch=torch.LongTensor(a_batch).cuda()
        # r_batch=torch.tensor(r_batch).cuda()
        # R_batch=torch.zeros(r_batch.shape).cuda()
        ...
        if self.model_type<2:
            with torch.no_grad():
                ...
                #v_batch=self.criticNetwork.forward(s_batch).squeeze().cuda()
